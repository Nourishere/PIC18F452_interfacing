...
STD_ReturnType lcd_intialize(const chr_LCD_t * lcd){
	STD_ReturnType ret = E_OK;
	uint8 i;
	if(NULL == lcd || (lcd -> bit_mode != 0 && lcd -> bit_mode != 1) || (E_NOT_OK == lcd_check_access(lcd)) )
		ret = E_NOT_OK;
	else{
		ret = GPIO_pin_intialize(&(lcd -> lcd_rs)); 
		ret = GPIO_pin_intialize(&(lcd -> lcd_en)); 
		if(!(lcd -> bit_mode)){ /* 4bit mode */ 
			for(i=0;i<4;i++)
			...
			ret =lcd_send_command(lcd,LCD_BASE_CUR_ADD); 
...
STD_ReturnType lcd_send_char_data_position(const chr_LCD_t * lcd, uint8 row, uint8 col, uint8 data){
	...
	if(NULL == lcd)
		...
		ret = lcd_set_cursor(lcd, row, col); //review this part
...
STD_ReturnType lcd_send_custom_char(const chr_LCD_t * lcd, uint8 row, uint8 col, const uint8 _chr[], uint8 mem_pos){
	...
	if(NULL == lcd)
		...
		ret = lcd_send_command(lcd, LCD_CGRAM_START + (--mem_pos *8)); //This just sets the CGRAM add
		for(;lc<=7;lc++)
...
/*@brief: Converts a byte (uint8) to a string for lcd operations.
...
/*@brief: Converts a byte (uint16) to a string for lcd operations.
...
/*@brief: Converts a byte (uint32) to a string for lcd operations.
...
static STD_ReturnType lcd_set_cursor(const chr_LCD_t *lcd, uint8 row, uint8 col){
	...
	col--;
	switch(row){
		case 1: lcd_send_command(lcd, (LCD_BASE_CUR_ADD + col)); break;
		case 2: lcd_send_command(lcd, (LCD_BASE_CUR_ADD + 40 + col)); break;
		case 3: lcd_send_command(lcd, (0x94 + col)); break;// I do not understand these parts
