...
static STD_ReturnType USART_baud_initialize(uint32 baud_rate, USART_baud_mode baud_mode);
...
STD_ReturnType USART_Tx_initialize(const USART_Tx_t* usart_tx){
	...
	ret = INT_USART_Tx_init(usart_tx -> priority);
#elif(INT_USART_TX == INT_EN && INT_PR == INT_DIS)
	ret = INT_USART_Tx_init(1); //Garbage value
#endif
	if(NULL== usart_tx)
		ret = E_NOT_OK;
	else{
		...
		USART_PIN_SERIAL();
		/* Set up baud rate */
		ret &= USART_baud_initialize(usart_tx -> baud_rate, usart_tx -> baud_mode);
...
STD_ReturnType USART_send_raw(const USART_Tx_t* usart_tx, uint16 data){
	...
	if(NULL == usart_tx)
		...
		uint8 k=0;
		ret &= USART_parity_generator(usart_tx -> parity, data, &k);
		TXSTA1bits.TX9D=k;
#else
		ret = E_NOT_OK;
#endif
...
#elif(INT_USART_TX == INT_EN && INT_PR == INT_DIS)
		INT_USART_Rx_init(1); //Garbage value
#endif
		/* Set up baud rate */
		ret = USART_baud_initialize(usart_rx -> baud_rate, usart_rx -> baud_mode);
		/* Set up pin RC6&RC7 as serial */	
		USART_PIN_SERIAL();
 ...
 * @param: A void pointer to either a USART_Tx_t type or a USART_Rx_t type.
 * 		   
 * @return: E_OK upon success and E_NOT_OK otherwise.
...
static STD_ReturnType USART_baud_initialize(uint32 baud_rate, USART_baud_mode baud_mode){
	...
	if (baud_rate == 0)
		...
		if(baud_mode == USART_high_speed){
			USART_HS(); 
#if (USART_MODE == _ASYNC)
				SPBRG = (uint8)((_XTAL_FREQ / ((baud_rate) * 16.0))-1);
#else
				ret = E_NOT_OK; /* HS mode does not work with synchronous UART */
#endif
		}
		else if(baud_mode == USART_low_speed){
			USART_LS(); 
#if (USART_MODE == _ASYNC)
				SPBRG =(uint8)((_XTAL_FREQ / ((baud_rate) * 64.0))-1);		
#elif (USART_MODE == _SYNC_MASTER || USART_MODE == _SYNC_SLAVE)
				SPBRG = (uint8)((_XTAL_FREQ / ((baud_rate) * 4.0))-1);		
#else
				ret = E_NOT_OK;
#endif
		}
