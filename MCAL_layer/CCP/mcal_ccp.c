...
STD_ReturnType CCP2_initialize(const CCP2_t * ccp2){
	...
	if(NULL == ccp2)
		...
		/* Configure the interrupt */
		#if (INT_PR == INT_EN)
		ret = ret && INT_CCP2_init(ccp2 -> priority);
		#elif (INT_PR == INT_DIS)
		ret = ret && INT_CCP2_initialize(1); // Garbage value
		#else 
		ret = E_NOT_OK;
		#endif
		/* Configure the mode */
		...
		/** Compare mode **/
			...
			uint16 presc_val=0;
			ret = TMR_PRESC2VAL(ccp2 -> op_mode.pwm.timer2.prescaler, &presc_val);
			/*@note: The formula uses the duty cycle in seconds, which the time the PWM is high.
 			 *       The duty cycle is converted to high_time (duty cycle given in seconds).
 			 */
			PR2 = (uint8) (((_XTAL_FREQ)/(4.0f * (ccp2 -> op_mode.pwm.PWM_freq) * (presc_val)))-1);
			float DC_dec = (ccp2 -> op_mode.pwm.PWM_duty_cycle) / 100.0f; /* Convert percentage to decimal */
			float high_time = ((DC_dec) / (ccp2 -> op_mode.pwm.PWM_freq)); /* Calculate high time */
			uint16 CCP2PR_val = (uint16) (( (high_time) * (_XTAL_FREQ)) / ( (presc_val) ));		
