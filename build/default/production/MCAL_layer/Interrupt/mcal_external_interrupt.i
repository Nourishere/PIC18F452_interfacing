...
STD_ReturnType INT_INTx_initialize(const INT_INTx_t *lint){
 ...
 if(((void*)0) == lint || (STD_ReturnType)(0x00) == INT_INTx_check_access(lint))
  ...
  ret = ret && INT_INTx_clear_flag(lint);

  ret = ret && GPIO_pin_direction_initialize(&(lint -> Ipin));

  ret = ret && INT_INTx_set_callback_routine(lint);





  ret = ret && INT_INTx_edge_initialize(lint);

  ret = ret && INT_INTx_enable(lint);
 }
 return ret;
}
...
# 158 "MCAL_layer/Interrupt/mcal_external_interrupt.c"
    INTCONbits.GIE=1;

    break;
   case(INT2_I):
    INTCON3bits.INT2IE=1;
# 171 "MCAL_layer/Interrupt/mcal_external_interrupt.c"
    INTCONbits.GIE=1;
